...
<Callout type="info">
	First public alpha. Core gameplay, multiplayer, and player accounts are all live.
</Callout>
...
- 3 BGM tracks (menu, solo, race) with auto-crossfade
- 9 SFX (jump, double jump, hit, game over, milestone, countdown, achievement, skin equip, victory)
- Adjustable BGM + SFX volume sliders; preferences saved to localStorage
...
- Bootstrap: `ADMIN_TWITCH_ID` env var for auto-promotion on first login
- CLI: `pnpm --filter @fangdash/api promote-admin <username>`
