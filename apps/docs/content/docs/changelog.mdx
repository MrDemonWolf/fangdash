---
title: Changelog
description: Gameplay changelog for FangDash — every change to the free multiplayer Twitch wolf runner, newest first, each linked to its commit.
keywords:
  - fangdash changelog
  - fangdash gameplay changes
  - fangdash commit history
  - fangdash multiplayer updates
...
<Callout type="info">
	Every entry links to the commit that introduced it. For official release notes, visit
...
- Hidden debug/cheat menu (admin) — set score & distance, invincibility, skip difficulty ([c4ab203](https://github.com/MrDemonWolf/fangdash/commit/c4ab203))
