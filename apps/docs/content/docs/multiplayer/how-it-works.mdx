...
FangDash uses a mulberry32 seeded PRNG algorithm. When a race starts, the server generates a random seed using `crypto.randomUUID()` and sends it to all players. The server also computes obstacle placements server-side to ensure consistency. This means:
