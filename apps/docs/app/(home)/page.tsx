import { Gamepad2, Layers, Swords, Target, Trophy, Tv, Zap } from "lucide-react";
...
const features = [
	{
		...
		description: "Jump, dodge, and dash through procedurally generated obstacle courses as a wolf.",
	},
	{
		icon: Swords,
		title: "Multiplayer Racing",
		description: "Race up to 4 players in real-time using WebSocket-powered rooms.",
	},
	{
		icon: Trophy,
		title: "Skins & Achievements",
		description: "Unlock 6 unique wolf skins and earn achievements through gameplay.",
	},
	{
		icon: Tv,
		title: "Twitch Integration",
		description: "Sign in with Twitch, race your chat, and show off your skins on stream.",
	},
];
		...
		<main className="flex flex-1 flex-col overflow-hidden">
			...
			<section className="relative flex flex-col items-center justify-center px-4 pt-32 pb-20 text-center">
				...
				<p className="mx-auto mb-10 max-w-lg text-lg leading-relaxed text-fd-muted-foreground">
					A fast-paced multiplayer endless runner where players race as wolves, dodging obstacles
					and competing for the highest score. Built for Twitch streamers and their communities.
				</p>
			...
			<section className="mx-auto flex w-full max-w-2xl flex-wrap items-center justify-center gap-8 px-6 py-8 sm:gap-16">
				{stats.map((stat) => (
					<div key={stat.label} className="flex items-center gap-3">
						<stat.icon className="h-5 w-5 shrink-0" style={{ color: "var(--color-fd-primary)" }} />
			...
			<section className="mx-auto grid w-full max-w-5xl gap-4 px-6 py-16 sm:grid-cols-2 lg:grid-cols-4">
				{features.map((feature) => (
					...
					>
						...
						>
							<feature.icon className="h-5 w-5" style={{ color: "var(--color-fd-primary)" }} />
						</div>
						<h3 className="mb-2 text-sm font-semibold text-fd-foreground">{feature.title}</h3>
