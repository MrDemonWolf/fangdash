...
import { player, playerSkin } from "../../db/schema";
import { ensurePlayer } from "../../lib/ensure-player";
import { SKINS, getSkinById, type SkinDefinition } from "@fangdash/shared";
...
export const skinRouter = router({
  getUnlockedSkins: protectedProcedure.query(async ({ ctx }) => {
    ...
    if (!playerRecord) return ["gray-wolf"];
  ...
  equipSkin: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const playerRecord = await ensurePlayer(ctx.db, ctx.user.id);
      if (!playerRecord) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
  ...
  getEquippedSkin: protectedProcedure.query(async ({ ctx }) => {
    const playerRecord = await ensurePlayer(ctx.db, ctx.user.id);
    if (!playerRecord) return { skinId: "gray-wolf" };
