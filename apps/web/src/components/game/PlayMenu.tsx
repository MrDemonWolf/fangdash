...
type Tab = "audio" | "skins" | "stats" | "leaderboard" | "controls" | "quit";
...
function AudioTab({ muted, volume, onToggleMute, onVolumeChange }: Pick<PlayMenuProps, "muted" | "volume" | "onToggleMute" | "onVolumeChange">) {
  return (
    <div className="space-y-6">
      <h3 className="text-sm font-mono uppercase tracking-widest text-[#0FACED]/60">Audio Settings</h3>
...
function SkinsTab({ isSignedIn }: { isSignedIn: boolean }) {
  ...
  const { mutate: equipSkin, isPending } = useMutation(
    trpc.skin.equipSkin.mutationOptions({
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: trpc.skin.getEquippedSkin.queryKey() });
      },
    })
  );
        ...
        {gallery?.map((skin) => {
          ...
          return (
            ...
            >
              ...
              {isEquipped && (
                <span className="absolute top-1.5 right-1.5 text-[10px] font-mono text-[#0FACED] font-bold">ON</span>
              )}
            </button>
          );
        })}
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        {[
          ...
          { label: "Total Distance", value: `${Math.floor(stats.totalDistance).toLocaleString()} m` },
          { label: "Obstacles Cleared", value: stats.totalObstaclesCleared.toLocaleString() },
        ].map(({ label, value }) => (
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-1">
        {entries?.map((entry) => (
          <div key={entry.scoreId} className="flex items-center gap-3 px-3 py-2 rounded border border-white/5 bg-white/3 hover:bg-white/5 transition-colors">
            <span className={[
              "w-6 text-center text-sm font-mono font-bold",
              entry.rank === 1 ? "text-yellow-400" : entry.rank === 2 ? "text-slate-300" : entry.rank === 3 ? "text-amber-600" : "text-white/30",
            ].join(" ")}>
              {entry.rank}
            </span>
            <span className="flex-1 text-sm font-mono text-white/80 truncate">{entry.username ?? "Anonymous"}</span>
            <span className="text-sm font-mono font-bold tabular-nums text-[#0FACED]">{entry.score.toLocaleString()}</span>
          </div>
        ))}
        {!entries?.length && (
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {[
          ...
          <div key={key} className="flex items-center justify-between gap-4 px-3 py-2 rounded border border-white/10 bg-white/5">
            <kbd className="px-2 py-1 rounded bg-white/10 text-xs font-mono text-white/70">{key}</kbd>
            <span className="text-sm text-white/60">{action}</span>
          </div>
        ))}
      </div>
      <p className="text-sm text-white/40 mt-4">Survive as long as possible. The game gets faster over time!</p>
    </div>
  );
}

export function PlayMenu({ onClose, muted, volume, onToggleMute, onVolumeChange, isSignedIn }: PlayMenuProps) {
  ...
  return (
    <div className="absolute inset-0 z-40 flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="relative w-full max-w-lg mx-4 rounded-xl border border-[#0FACED]/20 bg-[#091533]/95 shadow-[0_0_60px_rgba(15,172,237,0.15)] overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/10">
          <span className="text-base font-mono font-bold uppercase tracking-widest text-white/80">Menu</span>
          <button
            onClick={onClose}
        ...
        <div className="flex">
          {/* Sidebar tabs */}
          <div className="flex flex-col w-32 border-r border-white/10 py-3 shrink-0">
            {tabs.map((tab) => (
              <button
                key={tab.id}
                onClick={() => {
          ...
          <div className="flex-1 p-5 min-h-[280px]">
            {activeTab === "audio" && (
              <AudioTab muted={muted} volume={volume} onToggleMute={onToggleMute} onVolumeChange={onVolumeChange} />
            )}
